...
### Assign Static Public IP to VM

Bash script that automates making your VM's public IP static, by:

- Getting the public IP name using az network public-ip list

- Extracting the right name using grep/awk

- Updating it to static
...
echo "Fetching Public IP resource in resource group: $RESOURCE_GROUP..."

# Get the first public IP name in the resource group
...
if [ -z "$PUBLIC_IP_NAME" ]; then
  echo "No public IP resource found in resource group $RESOURCE_GROUP."
  exit 1
fi

echo "Found Public IP: $PUBLIC_IP_NAME"

echo "Updating allocation method to Static..."
az network public-ip update \
  --resource-group "$RESOURCE_GROUP" \
  --name "$PUBLIC_IP_NAME" \
  --allocation-method Static

echo "Public IP '$PUBLIC_IP_NAME' is now Static."

```

Execute the script:
```bash
chmod +x make_ip_static.sh
./make_ip_static.sh
```